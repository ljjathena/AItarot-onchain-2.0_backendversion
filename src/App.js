...
const SPREAD_LAYOUTS = {
  ...
  "求职评估牌阵": [
    ...
    { x: "75%", y: "25%", label: "求职趋势" },         // P5 右上（你叫“求职趋势”，我按你现在的文案）
  ...
  // 「工作二选一牌阵」5张：A区(左两张)/B区(右两张)/你自己在下方中间
  "工作二选一牌阵": [
  ...
  // 我把它重排成视觉上接近书里：1和2在左，3和6在右，5在中间，4在最上
  "突破暧昧牌阵": [
  ...
  // 我设计成资金流动 / 漏洞 / 机会 / 风险 / 最终走向，视觉是横向+下方汇总
  "财富流动牌阵": [
...
function drawCards(num) {
  ...
  if (drawMode === "auto") {
    ...
    // 写回到你现有的结果状态（沿用你原来的字段：drawnCards）
    setDrawnCards(picks);

    // 自动模式下直接翻面（若你在其他地方用到了 revealed，可同步设置）
    setRevealed(Object.fromEntries(picks.map((_, i) => [i, true])));

    // 你的 AI 逻辑保持不变：此处不强制调用，仍由“免费AI解读 / 支付解锁”按钮触发
    // 如果你想抽完就立刻跑 AI，可在此调用 handleAI()。
  } else {
...
async function interpretManual() {
  ...
  // 你如果要求“必须都已翻面再解读”，可以加校验
  // const allOpen = [...Array(need).keys()].every(i => revealed[i]);
  // if (!allOpen) { alert("请先把所有牌翻面"); return; }

  // 把手动结果塞回你原先的“本轮抽中卡组”state
   setDrawnCards(manualPicks);
 await handleAI();
}
...
if (paidRun && okStable) {
  ...
  // 这一次到底算不算“付费占卜的那一单”
  // 规则：只要本次是通过付费触发的，或者我们还在付费流程里，或者这轮有付费额度(credit>0n)，
  // 就当成付费失败，给退款通道，而不是当成免费。
  // 这一次到底算不算“付费占卜的那一单”
// 现在我们只信 fromPaid 这个参数：
...
function startManualRound() {
  ...
  //重置一轮
  resetRoundStates();        // 这会清很多状态（你已有的函数）
...
>
  ...
  {phase === "done" && (
    <>
      {/* 摆阵展示区域：按牌阵几何形状把牌摆进来（这是你原本的逻辑，几乎原样拷贝） */}
      {drawnCards.length > 0 && (
        <div
          style={{
            position: "relative",
